...
function oldMomentFormat(mom, formatStr, isJalaali) {
    ...
    return oldMomentProto.format.call(mom, toJalaaliUnit(formatStr, isJalaali)).replace(/j/g, "");
}
...
function formatDateWithChunk(date, chunk, isJalaali) {
	...
	if (typeof chunk === 'string') { // a literal string
		...
		if (tokenOverrides[token]) {
		    ...
		    return tokenOverrides[token](date, isJalaali);
		}
	...
	if (typeof chunk === 'string') { // a literal string
		...
		if (unit && unzonedDate1.isSame(unzonedDate2, toJalaaliUnit(unit, isJalaali))) {
		   // return oldMomentFormat(date1, token); // would be the same if we used `date2`
		    return oldMomentFormat(date1, token, isJalaali);
...
var Grid = FC.Grid = Class.extend(ListenerMixin, MouseIgnorerMixin, {
	...
	constructor: function(view) {
		...
		this.basePrice = view.opt('basePrice');
		this.currencyUnit = view.opt('currencyUnit');
		this.elsByFill = {};
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
    ...
    //+(this.isJalaali  ? persianNumber(date.jDate()) :  date.date());
        ...
        //this add by alireza to show base price
	    html += '<div class="fc-base-price">' + this.basePrice + ' ' +this.currencyUnit+'</div>';
		html += '</td>';
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	constructor: function(calendar, type, options, intervalDuration) {
		...
		this.basePrice = this.opt('basePrice');
		this.currencyUnit = this.opt('currencyUnit');
